...
Running on thread: DefaultDispatcher-worker-1 @coroutine#2
```
#### Code Example Performing Complex Computations
```kotlin
import kotlinx.coroutines.*
...
fun main() = runBlocking {
    ...
    val sumOfSquares = withContext(Dispatchers.Default) {
        println("Thread on which we are performing computations : ${Thread.currentThread().name}")
        numbers.map { it * it }.sum()
    }
    println("Finished computations on thread: ${Thread.currentThread().name}")
...
```
#### Code Example Fetching data from mongodb cluster
```kotlin
...
suspend fun fetchDataFromMongoDB(): List<Document> = withContext(Dispatchers.IO) {
    println("Current thread is ${Thread.currentThread().name}")
    // Replace with your MongoDB connection string
    val connectionString = "mongodb+srv://muhammadwaqas:waqas1234@cluster0.itzzwbn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0\n"
...
fun main() = runBlocking {
    // Fetch data in the background using Dispatchers.IO
    println("Thread ${Thread.currentThread().name}")
    val documents = fetchDataFromMongoDB()
